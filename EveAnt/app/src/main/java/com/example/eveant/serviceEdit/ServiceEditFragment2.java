...
public class ServiceEditFragment2 extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        view.findViewById(R.id.previous_button).setOnClickListener(v -> {
            ((ServiceEditFragment) requireParentFragment()).replaceFragment(new ServiceEditFragment1());
        });

        view.findViewById(R.id.next_button).setOnClickListener(v -> {
            ((ServiceEditFragment) requireParentFragment()).replaceFragment(new ServiceEditFragment3());
        });
